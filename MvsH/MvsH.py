...
for i in runs: 
    ...
    plt.errorbar(H, M, yerr = Err, label = '{} K'.format(T), marker = '.', linestyle = 'none')
...
plt.legend()
if savepic:
    plt.savefig(MHDir+'{}_MvsH_emu_oe.pdf'.format(comp))  
...
for i in MHdata.keys():
    ...
    H = oeToTesla(H)
    Err = emuToBohr2(Err)
    plt.errorbar(H,M, yerr = Err,label = '{} K'.format(i), marker = '.', linestyle = 'none')
plt.title(comp)
plt.ylabel('Moment (\N{GREEK SMALL LETTER MU}B) {}^-1'.format(per))
plt.xlabel('Field (T)')
plt.legend()
if savepic:
    plt.savefig(MHDir+'{}_MvsH_uB_T.pdf'.format(comp))  
plt.show()
